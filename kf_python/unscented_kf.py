import numpy as np
import kf_python.unscented_transform as ut
...
class UnscentedKF:
    def __init__(self, f, h, R, Q, L, alpha, beta, kappa):
        ...
        self.beta = beta
        self.kappa = kappa
        self._lambda = ut.calculate_lambda(L=self.L, alpha=self.alpha, k=self.kappa)
        wm, wc = ut.calc_weights(alpha=alpha, beta=beta, L=L, _lambda=self._lambda)
        self.wm = wm
        self.wc = wc

    def predict(self, prev_mean, prev_cov, u, trans_f, delta_t=1):
        # Calculate sigma points for state
        sigma_points_state = ut.calc_sigma_points(x_mean=prev_mean, x_cov=prev_cov, _lambda=self._lambda)
        ...
        return predict_state_mean, predict_state_cov, dif_state, sigma_points_state_propagated

    def update(self, predict_mean, predict_cov, dif_state, z, measurement_f, sp_propagated):
        # Calculate sigma points for observation
        # sigma_points_obs= ut.calc_sigma_points(x_mean=predict_mean, x_cov=predict_cov,
    ...
    def propagate(self, mean: np.ndarray, cov: np.ndarray, u: np.ndarray, trans_f, z: np.ndarray,
        ...
        predicted_next_mean, predicted_next_cov, dif, sp_propagated = self.predict(mean, cov, u, trans_f, delta_t_u)
        return self.update(predicted_next_mean, predicted_next_cov, dif, z, measurement_f, sp_propagated)

    def default_run(self, x_0, cov_0, controls, observations, num_iterations):
